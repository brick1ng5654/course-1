...
float find_year(MOV *current_node);

float find_plr(MOV *current_node);

void find_by(HD *ph, float (*func_name)(MOV*)); /* Find by year or PLR */
...
int main() {
    ...
    do{
        ...
        case 2:{
            puts("\nYour selection is DELETE THE DATA BY YEAR\n");
            output_list(ph);
            find_by(ph,find_year);
            getchar();
            break;
        }

        case 3:{
            puts("\nYour selection is DELETE THE DATA BY PLR\n");
            output_list(ph);
            find_by(ph,find_plr);
            getchar();
            break;
...
void menu(){
    ...
    puts("2 - for DELETE THE DATA BY YEAR");
    puts("3 - for DELETE THE DATA BY PLR");
    puts("4 - for RESET TO ORIGINAL DATA");
...
float find_year(MOV *current_node){
    return current_node->year;
}

float find_plr(MOV *current_node){
    return current_node->plr;
}

void find_by(HD *ph, float (*func_name)(MOV*)) {
    MOV *current, *temp;
    int count=0;
    float n;
    printf("\nEnter a field value to remove elements of list with the same field value: ");
    scanf("%f", &n);

    current = ph->first;
    while(current != NULL) {
        if(n == func_name(current)) {
            temp = current;
            current = current->next;
            delete_node(ph, temp);
            count++;
    ...
    if (count==0){
            printf("\nThere is no field that include entered value\n");
    } else printf("\nAll elements, which include entered value in chosen field were successfully removed from list\n");
}
